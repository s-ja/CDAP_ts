function testFunction2(x: number): number {
  return x * 2;
}

function testFunction3(x?: number): void {
  1 + 1;
  // return x * 3; // Error
}
...
function testFunction4(x: number | string) {
  // *type narrowing
  // if (typeof x == number) {
  //   return x + 2;
  // }
...
function hello(x: string | undefined): void {
  if (x) {
    console.log(`hello ${x}`);
  } else {
    console.log(`no name`);
...
function characterCounter(x: number | string): number {
  return x.toString().length;
}

function characterCounter2(x: number | string): number {
  if (typeof x == "number") {
    return x.toString().length;
  } else {
    return x.length;
...
function marryAvailability(x: number, y: boolean, z: string): string | void {
  let score: number = x;
  if (y === true) {
    score = score + 500;
  } else if (z === "상") {
