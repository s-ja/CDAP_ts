...
let bowWow: (string | boolean)[] = ["dog", true, "asdf", false];

// tuple type
let bowWow2: [string, boolean, number?] = ["dog", true];
// let bowWow2: [string, boolean?, number] = ["dog", 123]; // !Error

function fnc3(...a: number[]) {
  console.log(a); // => array
}

fnc3(1, 2, 3, 4, 5, 6);

function fnc4(...a: [number, string]) {
  console.log(a); // => array
}

fnc4(1, "asd");

function fnc5(a: number, b: string) {
  console.log([a, b]);
}

let arr4 = [1, 2, 3];
let arr5 = [4, 5, 6];
let arr6: [...number[]] = [...arr4, ...arr5];
...
function 숙제3(...a: [string, boolean, ...(number | string)[]]) {}
...
function 숙제4(...a: (string | number)[]): [string[], number[]] {
  let strArray: string[] = [];
  let numArray: number[] = [];
  a.map((value) => {
...
// ? solution
function 숙제4_solution(...rest: (string | number)[]) {
